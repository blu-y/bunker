...
namespace frontier_exploration {
	...
	output.minAngle = std::numeric_limits<double>::infinity();	// revised by pileun 20231207
...
//                    output.middle.y = wy;
//                }
		// revised by pileun 20231207
		double ang = atan2(double(referenceY) - double(wy), double(referenceX) - double(wx));
		double ang_diff_tmp = ang - _heading;
		double ang_diff = 0;		
		if (ang_diff_tmp > PI)    
		    ang_diff = ang_diff_tmp - 2*PI;
		else if (ang_diff_tmp < -PI)
		    ang_diff = ang_diff_tmp + 2*PI;
		else
		    ang_diff = ang_diff_tmp;

		if (abs(ang_diff) < output.minAngle)	
                {
                    output.minDistance = distance;
		    output.minAngle = ang_diff;
                    output.middle.x = wx;
                    output.middle.y = wy;
                }
    ...
    void FrontierSearch::updateHeading(double data)
    {
	_heading = data;
    }
...
//        auto nHood = nhood4(idx, *_costmap);
	auto nHood = nhood8(idx, *_costmap);		// revised by pileun 20240107
...
//               (_gainScale * frontier.size * _costmap->getResolution());
	// revised by pileun 20231207
	return (_potentialScale * frontier.minAngle * _costmap->getResolution()) ;//-
//               (_gainScale * frontier.size * _costmap->getResolution());
    }
} // namespace frontier_exploration
