...
#		self.slam_sub = rospy.Subscriber('odometry/filtered', Odometry, self.slamCb)
	...
	def slamCb(self,data):
		self.slam = data
		y = self.slam.pose.pose.position.x
		z = self.slam.pose.pose.position.y
		x = self.slam.pose.pose.position.z
		qy = self.slam.pose.pose.orientation.x
		qz = self.slam.pose.pose.orientation.y
		qx = self.slam.pose.pose.orientation.z
		qw = self.slam.pose.pose.orientation.w
#		orientation_list = [self.slam.pose.pose.orientation.x, self.slam.pose.pose.orientation.y, self.slam.pose.pose.orientation.z, self.slam.pose.pose.orientation.w]
		...
		dist = 0
	        current_pose = self.slam.pose.pose.position
...
#		    current_height = current_pose.y
	        ...
	        scan_interval = math.sqrt( (self.pre_scan_pose[0]-current_pose.z)**2 + (self.pre_scan_pose[1]-current_pose.x)**2 + (self.pre_scan_pose[2]-current_pose.y)**2 )
	        print scan_interval, dist, self.mean_dist 
	        if ( (scan_interval > 5) and (dist > self.mean_dist) ):
#		    count = count + 1
		    self.scanPositions.append([current_pose.z, current_pose.x, current_pose.y])
		    header = Header()
		    header.frame_id = "/map"
		    pc2 = point_cloud2.create_cloud(header, fields, self.scanPositions)
		    pc2.header.stamp = rospy.Time.now()
		    self.pub_scan_pose.publish(pc2)
		    self.pre_scan_pose = [current_pose.z, current_pose.x, current_pose.y]
		    print scan_interval
