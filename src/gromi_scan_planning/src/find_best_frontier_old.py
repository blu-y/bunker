...
def MakeGroundFrontier(data):
    global pts, current_pose, pre_pose, fields, pre_heading, leftWheelVel, rightWheelVel
...
#	heading = math.atan2(current_pose.x-pre_pose[0], current_pose.y-pre_pose[2]) * (180/math.pi)
	heading = int(leftWheelVel)/(int(rightWheelVel)+1)
...
def LeftWheelVel(data):
    global leftWheelVel
    leftWheelVel = data

def RightWheelVel(data):
    global rightWheelVel
    rightWheelVel = data
...
rospy.Subscriber('/left_wheel_vel', Int8, LeftWheelVel)
rospy.Subscriber('/right_wheel_vel', Int8, RightWheelVel)
