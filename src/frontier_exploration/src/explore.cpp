...
namespace frontier_exploration {
	...
	ROS_INFO("planner_frequency,  progress_timeout, potential_scale, gain_scale, minimum_frontier_size is (%.3f, %.3f, %.3f, %.3f, %.3f)", _plannerFrequency, timeout, _potentialScale, _gainScale, minFrontierSize);

	_headingSub = _nh->subscribe<std_msgs::Float32>("/current_heading", 1000, &Explore::updateHeading, this);
    ...
    // revised by pileun 20231207
    void Explore::updateHeading(const std_msgs::Float32::ConstPtr& msg)
    {
	_search.updateHeading(msg->data);
	_heading = msg->data;
    }
    ...
    void Explore::makePlan()
    {
        ...
        // revised by pileun 20231207
	ROS_INFO("current pose and heading is (%.3f, %.3f, %.3f)", pose.position.x, pose.position.y, _heading);
